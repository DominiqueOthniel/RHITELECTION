  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (verifyError) {
        ...
        // Mettre à jour la dernière connexion (ne doit pas bloquer la connexion si RPC refusée)
        // @ts-ignore - Type issue avec Supabase RPC
        const { error: loginRpcError } = await supabase.rpc('update_admin_last_login', {
          p_admin_id: (data[0] as any).id
        } as any)
        if (loginRpcError) {
          console.warn('update_admin_last_login:', loginRpcError)
        }
